...
class UsersControllerTest {
    ...
    void testLogin3() throws Exception {
        when(tokenService.generateToken(Mockito.<User>any())).thenThrow(new AccountExpiredException("U.U.U"));
        when(authenticationManager.authenticate(Mockito.<Authentication>any()))
                .thenReturn(new TestingAuthenticationToken(new User(), "Credentials"));
        MockHttpServletRequestBuilder contentTypeResult = MockMvcRequestBuilders.post("/users/login")
                .contentType(MediaType.APPLICATION_JSON);

        ObjectMapper objectMapper = new ObjectMapper();
        MockHttpServletRequestBuilder requestBuilder = contentTypeResult
                .content(objectMapper.writeValueAsString(new AutenticarDTO("jane.doe@example.org", "Senha")));
        ResultActions actualPerformResult = MockMvcBuilders.standaloneSetup(usersController)
                .build()
